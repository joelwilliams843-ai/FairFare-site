...
# MongoDB connection - optional for FairFare (not needed for core functionality)
...
if mongo_url:
    try:
        ...
        db = client[os.environ.get('DB_NAME', 'fairfare')]
    except Exception as e:
        logging.warning(f"MongoDB connection failed (non-critical): {e}")
...
async def places_autocomplete(request: PlacesAutocompleteRequest):
    ...
    # Add location bias if provided (16km radius = ~10 miles for local business searches)
    # Using locationBias instead of locationRestriction to still allow broader results as fallback
    if request.location_lat and request.location_lng:
